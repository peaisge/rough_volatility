...
eta = 2.2
m = 2
times = np.arange(1, 201) / 365 # intervalles de temps
times_discretized = np.arange(1, 200*m +1) / (365 * m) # discr̩tisation des intervalles de temps
N = len(times)
...
fWW = lambda i, j: covW_fun(times_discretized[i], times_discretized[j])
fZZ = lambda i, j: min(times_discretized[i], times_discretized[j])
fWZ = lambda i, j: covWZ_fun(times_discretized[i], times_discretized[j])
...
cov_matrix = np.asarray(np.bmat([[covWW, covWZ], [covWZ.T, covZZ]]),dtype='float64') # matrice des covariances du vecteur (W, Z)

T_simul = times

np.savetxt("cov2.csv", cov_matrix, delimiter=",")
...
xi_0 = np.asarray([.02]*len(T_simul))
...
def simul(xi=xi_0):
    ...
    xi_1 = np.insert(np.repeat(xi,m),0,.02)
    v_sample = xi_1 * np.exp(eta * W_sample - H * (eta**2) * integrande_sample)
...
ATM_skews_emp = []
for i in range(25):
    index = next((j for j, x_j in enumerate(T_simul) if x_j == np.array(T[i],dtype='float32')),-1)
    if (index!=-1):
        log_moneyness = np.log(np.asarray(K[i])/np.asarray(F[i]))
        if (i<25):
            axes[i//5,i%5].plot(log_moneyness[::4],smiles_bid_list[i][::4],'b*')
            axes[i//5,i%5].plot(log_moneyness[::4],smiles_ask_list[i][::4],'r*')
            ...
            axes[i//5,i%5].plot(x,[np.sqrt(svi(y,params_list[index][0])) for y in x],color="orange")
            axes[i//5,i%5].set_title("T = %.3f"%T[i])
            axes[i//5,i%5].set_ylim(0, 1)
            axes[i//5,i%5].locator_params(nbins=5, axis='x')
...
for i in range(25):
    ...
    if (index!=-1):
        ...
        if (i<25):
            plt.figure()
            plt.plot(log_moneyness[::4],smiles_bid_list[i][::4],'b*')
            plt.plot(log_moneyness[::4],smiles_ask_list[i][::4],'r*')
            best_params = least_squares(error, params0, args=(log_moneyness, (np.asarray(smiles_mid_list[i])**2)),bounds=([0,0,-1,-1,0],[5,20,1,1,10]),method='trf').x
            plt.plot(x,[np.sqrt(svi(y,params_list[index][0])) for y in x],color="orange")
            plt.title("T = %.3f"%T[i])
            plt.ylim(0, 1)
            plt.xlim(-0.4,0.2)
...
f = splrep(T_simul,swaps*T_simul,k=5,s=4)
plt.figure()
plt.plot(T_simul, swaps*T_simul, label="noisy data")
plt.plot(T_simul, splev(T_simul,f), label="fitted")
plt.plot(T_simul, splev(T_simul,f,der=1)/10, label="1st derivative")
plt.show()

xi = splev(T_simul,f,der=1)/10
xi = np.clip(xi,0.008,1)
plt.figure()
plt.plot(T_simul,xi)
plt.show()
